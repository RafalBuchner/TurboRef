...
class TurboReference:
	...
	def __init__(self):
		self.canvas = TurboCanvas(__DEF_CANVASSIZE__)
		self.path = None
	...
	def load(self, path):
		...
		self.canvas = TurboCanvas((canvasSize['width'], canvasSize['height']))
	...
	def __loadElementFromDict(self, data):
		...
		if data.get("type") == "image":
			...
			return TurboImage(path, position, scale, rotation, groupnames)
		if data.get("type") == "textbox":
			...
			return TurboTextBox(text, position, fontProperties, scale, rotation, groupnames)
...
class TurboCanvas:
	...
	def addImage(self, path, position, scale=1, rotation=0):
		self._addElement( 
				TurboImage(path, position, scale, rotation)
			)
	...
	def addTextBox(self, text, position, fontProperties=None, scale=1, rotation=0):
		...
		self._addElement( 
				TurboTextBox(text, position, fontProperties, scale, rotation)
			)
